...
func TestClearState(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		t.Run(tc.description, func(t *testing.T) {
			...
			state1, err := NewCheckpointState(stateDirectoryConfig, cpuPluginStateFileName, policyName, tc.cpuTopology, false, GenerateMachineStateFromPodEntries, metrics.DummyMetrics{})
			as.Nil(err)
			...
			state2, err := NewCheckpointState(stateDirectoryConfig, cpuPluginStateFileName, policyName, tc.cpuTopology, false, GenerateMachineStateFromPodEntries, metrics.DummyMetrics{})
			as.Nil(err)
...
func TestCheckpointStateHelpers(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.description, func(t *testing.T) {
			...
			state, err := NewCheckpointState(stateDirectoryConfig, cpuPluginStateFileName, policyName, tc.cpuTopology, false, GenerateMachineStateFromPodEntries, metrics.DummyMetrics{})
			as.Nil(err)
...
func TestGetNUMABindingRequestedQuantity(t *testing.T) {
	t.Parallel()

	newAllocationInfo := func(podUID, containerName, containerType, ownerPoolName, qosLevel string, numaBinding bool, request float64, annotations map[string]string) *AllocationInfo {
		allocationAnnotations := map[string]string{
			consts.PodAnnotationQoSLevelKey: qosLevel,
		}
		if numaBinding {
			allocationAnnotations[consts.PodAnnotationMemoryEnhancementNumaBinding] = consts.PodAnnotationMemoryEnhancementNumaBindingEnable
		}
		for k, v := range annotations {
			allocationAnnotations[k] = v
		}
	...
	nodeState := &NUMANodeState{
		PodEntries: PodEntries{
			"snb-pod": ContainerEntries{
				"main": newAllocationInfo("snb-pod", "main", pluginapi.ContainerType_MAIN.String(),
					commonstate.PoolNameShare, consts.PodAnnotationQoSLevelSharedCores, true, 1.25,
					map[string]string{
						cpuconsts.CPUStateAnnotationKeyNUMAHint: "0",
					}),
				"sidecar": newAllocationInfo("snb-pod", "sidecar", pluginapi.ContainerType_SIDECAR.String(),
			...
			"aggregated-pod": ContainerEntries{
				"main": newAllocationInfo("aggregated-pod", "main", pluginapi.ContainerType_MAIN.String(),
					commonstate.PoolNameShare, consts.PodAnnotationQoSLevelSharedCores, true, 1,
					map[string]string{
						consts.PodAnnotationAggregatedRequestsKey: "{\"cpu\":\"3\"}",
						cpuconsts.CPUStateAnnotationKeyNUMAHint:   "0",
					}),
				"sidecar": newAllocationInfo("aggregated-pod", "sidecar", pluginapi.ContainerType_SIDECAR.String(),
					commonstate.PoolNameShare, consts.PodAnnotationQoSLevelSharedCores, true, 1, map[string]string{
						cpuconsts.CPUStateAnnotationKeyNUMAHint: "0",
					}),
			},
			"legacy-snb-pod": ContainerEntries{
				"main": newAllocationInfo("legacy-snb-pod", "main", pluginapi.ContainerType_MAIN.String(),
					commonstate.PoolNameShare, consts.PodAnnotationQoSLevelSharedCores, true, 1.5, nil),
			},
			"non-binding-pod": ContainerEntries{
				"main": newAllocationInfo("non-binding-pod", "main", pluginapi.ContainerType_MAIN.String(),
					commonstate.PoolNameShare, consts.PodAnnotationQoSLevelSharedCores, false, 10, nil),
			},
			"dedicated-pod": ContainerEntries{
				"main": newAllocationInfo("dedicated-pod", "main", pluginapi.ContainerType_MAIN.String(),
					commonstate.PoolNameDedicated, consts.PodAnnotationQoSLevelDedicatedCores, true, 10,
					map[string]string{
						cpuconsts.CPUStateAnnotationKeyNUMAHint: "0",
					}),
			},
			"reclaimed-pod": ContainerEntries{
				"main": newAllocationInfo("reclaimed-pod", "main", pluginapi.ContainerType_MAIN.String(),
					commonstate.PoolNameReclaim, consts.PodAnnotationQoSLevelReclaimedCores, true, 2, map[string]string{
						cpuconsts.CPUStateAnnotationKeyNUMAHint: "0",
					}),
			},
			"non-actual-reclaimed-pod": ContainerEntries{
				"main": newAllocationInfo("non-actual-reclaimed-pod", "main", pluginapi.ContainerType_MAIN.String(),
					commonstate.PoolNameReclaim, consts.PodAnnotationQoSLevelReclaimedCores, true, 10, nil),
			},
			commonstate.PoolNameShare: ContainerEntries{
				commonstate.FakedContainerName: newAllocationInfo(commonstate.PoolNameShare, commonstate.FakedContainerName,
					pluginapi.ContainerType_MAIN.String(), commonstate.PoolNameShare, consts.PodAnnotationQoSLevelSharedCores,
					true, 10, nil),
			},
		},
	}

	requested := nodeState.GetNUMABindingRequestedQuantity("")
	require.Equal(t, 16.25, requested)

	requested = nodeState.GetNUMABindingRequestedQuantity("snb-pod")
	require.Equal(t, 14.5, requested)

	requested = (*NUMANodeState)(nil).GetNUMABindingRequestedQuantity("")
	require.Equal(t, 0.0, requested)
}
