...
func TestCalculateRampUpReclaimTarget(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "ratio applies to every ramp-up workload",
			ratio:    0.8,
			eligible: 20,
			reserve:  1,
			cap:      18,
		...
		{
			name:     "invalid ratio always rejects",
			ratio:    1.1,
			eligible: 20,
			reserve:  2,
			cap:      10,
		...
		{
			name:     "zero ratio and empty reserve stays empty",
			ratio:    0,
			eligible: 20,
			reserve:  0,
			cap:      10,
		...
		{
			name:     "zero ratio does not synthesize floor for one eligible cpu",
			ratio:    0,
			eligible: 1,
			reserve:  0,
			cap:      10,
