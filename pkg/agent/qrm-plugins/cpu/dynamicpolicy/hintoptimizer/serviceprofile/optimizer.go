...
func (p *serviceProfileHintOptimizer) optimizeHints(
	...
	general.Infof("origin hints: %v", hints)
	...
	general.Infof("optimize hints successfully, optimized hints: %v", hints)
	return nil
}
...
func (p *serviceProfileHintOptimizer) score(
	...
	for resourceName, weight := range p.resourceWeight {
		...
		general.Infof("scores for resource %s before normalize: %v", resourceName, scores)
		...
		general.Infof("scores for resource %s after normalize: %v", resourceName, scores)
