/*
Copyright 2022 The Katalyst Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package resourcepool

import (
	"context"
	"testing"

	"github.com/stretchr/testify/require"
	"k8s.io/apimachinery/pkg/api/resource"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"

	nodev1alpha1 "github.com/kubewharf/katalyst-api/pkg/apis/node/v1alpha1"
	npd "github.com/kubewharf/katalyst-core/pkg/metaserver/npd"
	rputil "github.com/kubewharf/katalyst-core/pkg/util/resource-pool"
)

func generateTestNPD(nodeName string) *nodev1alpha1.NodeProfileDescriptor {
	maxAggr := nodev1alpha1.AggregatorMax
	minAggr := nodev1alpha1.AggregatorMin
	return &nodev1alpha1.NodeProfileDescriptor{
		ObjectMeta: metav1.ObjectMeta{Name: nodeName},
		Status: nodev1alpha1.NodeProfileDescriptorStatus{
			NodeMetrics: []nodev1alpha1.ScopedNodeMetrics{
				{
					Scope: rputil.MetricScope,
					Metrics: []nodev1alpha1.MetricValue{
						{
							MetricName: "cpu",
							MetricLabels: map[string]string{
								"pool-name": "L1",
							},
							Value:      resource.MustParse("8"),
							Aggregator: &maxAggr,
						},
						{
							MetricName: "cpu",
							MetricLabels: map[string]string{
								"pool-name": "L1",
							},
							Value:      resource.MustParse("1"),
							Aggregator: &minAggr,
						},
						{
							MetricName: "memory",
							MetricLabels: map[string]string{
								"pool-name": "L1",
								"numa-id":   "0",
							},
							Value:      resource.MustParse("16Gi"),
							Aggregator: &maxAggr,
						},
					},
				},
			},
		},
	}
}

func TestNewResourcePoolManager(t *testing.T) {
	t.Parallel()

	mgr := NewResourcePoolManager(npd.NewDummyNPDFetcher())
	require.NotNil(t, mgr)
}

func TestConvertNPDResourcePools(t *testing.T) {
	t.Parallel()

	mgr := &resourcePoolManager{}
	pools, err := mgr.ConvertNPDResourcePools(generateTestNPD("node-a"))
	require.NoError(t, err)
	require.NotNil(t, pools)
	require.Len(t, pools, 2)
	require.Contains(t, pools, rputil.NumaIDAll)
	require.Contains(t, pools, 0)
}

func TestConvertNPDResourcePoolsNil(t *testing.T) {
	t.Parallel()

	mgr := &resourcePoolManager{}
	pools, err := mgr.ConvertNPDResourcePools(nil)
	require.NoError(t, err)
	require.Nil(t, pools)
}

func TestNodeResourcePools(t *testing.T) {
	t.Parallel()

	mgr := &resourcePoolManager{
		fetcher: &npd.DummyNPDFetcher{
			NPD: generateTestNPD("node-a"),
		},
	}
	pools, err := mgr.NodeResourcePools(context.Background())
	require.NoError(t, err)
	require.NotNil(t, pools)
	require.Len(t, pools, 2)
}
