...
func TestGetReclaimedNUMAHeadroomAllowsPercentageAboveOneHundred(t *testing.T) {
	got := GetReclaimedNUMAHeadroom(
		map[int]float64{0: 1},
		newDynamicWithPercentages(map[string]int{"consumer-a": 150}),
		"consumer-a",
	)
	want := map[int]float64{0: 1.5}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("GetReclaimedNUMAHeadroom = %#v, want %#v", got, want)
	}
}
