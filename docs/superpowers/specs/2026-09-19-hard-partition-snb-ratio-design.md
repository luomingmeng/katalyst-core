...
The decision is made from the immutable configuration captured for the current
advisor attempt. It is not re-read independently by validation, block planning,
floor derivation, or commit preparation.
...
`DynamicPolicy` selects the ratio through one policy-level helper using the
attempt's cloned dynamic configuration:
...
The attempt snapshot deep-copies mutable configuration fields used by CPU
planning. The same `advisorAttemptConfiguration` supplies the hard-partition
predicate, ratio, and reclaim-floor configuration throughout one response
attempt. A concurrent dynamic-config update therefore applies to the next
attempt rather than splitting one candidate across two configurations.
...
- One advisor attempt reads and clones its configuration once; mutation of the
  live configuration after capture does not change that attempt's ratio or
  floor inputs.
