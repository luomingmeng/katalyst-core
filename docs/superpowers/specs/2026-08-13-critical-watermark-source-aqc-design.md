...
`memory-advisor-critical-watermark-source`. `MemoryGuardOptions` stores the
string form at the CLI boundary and uses `StringVar` to preserve normal flag
behavior. `ApplyTo` normalizes an explicit empty value to `low` for backward
compatibility, accepts `low` or `high`, converts to the API
`CriticalWatermarkSource` type, and writes the typed result into the default
dynamic Memory Guard configuration. Validation errors remain lowercase.
