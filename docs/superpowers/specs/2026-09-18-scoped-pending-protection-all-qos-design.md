...
Implemented and reviewed on 2026-09-20. Linux and E2E validation for the
...
The plugin resolves each pending Pod once, not once per container, in this
order:

1. Use the existing Pod relative cgroup path when it is available.
2. Otherwise force one strict fresh kubelet Pod-cache synchronization; an
   ordinary cached lookup is insufficient during cold admission. A live Pod
   supplies its native Kubernetes QoS class. Ask `pkg/util/cgroup/common` for
   the pure candidates implied by every configured canonical root for that
   class. The filter recognizes both cgroupfs roots, such as
   `kubepods/burstable`, and systemd roots, such as
   `kubepods.slice/kubepods-burstable.slice`. Candidate generation performs no
   filesystem lookup.
3. If the QoS-filtered set has one candidate, retain it even before
   materialization. If it has multiple candidates, prefer one uniquely
   materialized path; if none is materialized, keep the live Pod pending and
   let the built topology DAG select the unique candidate whose
   controlled-primary ancestor is deepest.
4. If the strict fresh lookup reports that the Pod is absent, observe every
   allowed candidate. Classify the checkpoint allocation as stale only when all
   observations return typed `ENOENT` (`errors.Is(err, os.ErrNotExist)`). A
   unique existing candidate is retained, multiple existing candidates are
   ambiguous, and any non-`ENOENT` observation error is returned unchanged.
5. Fail closed when no DAG candidate matches, distinct candidates tie at the
   deepest ancestor depth, a candidate is outside the configured roots, or a
...
A live unresolved scope is not stale merely because its leaf has not yet been
materialized. It proceeds only when the DAG proves one controlled scope. The
old fallback that widens every primary node is deleted rather than retained as
a compatibility path.

All pending containers from one Pod consume the same fresh resolution result.
Their CPU sets are unioned only after that resolution, preventing mixed QoS or
scope observations within one admission attempt.

### Test Isolation

The candidate-root registry is process-global. Tests that need a different
root layout run in an isolated subprocess instead of mutating that registry
alongside parallel tests. Lower-level cgroup path and mount tests inject their
filesystem and mount resolvers per instance. This preserves parallel test
reliability without introducing production-only mutable hooks.
...
| live Pod, no materialized candidate | resolve uniquely through the controlled DAG or fail closed |
| fresh Pod absent, all candidates typed `ENOENT` | stale checkpoint allocation; skip protection |
| fresh Pod absent, candidate observation returns another error | preserve the error and fail closed |
...
11. cgroupfs and systemd configured roots produce QoS-specific candidates.
12. Candidate observation treats only typed `ENOENT` as absence and preserves
    permission, I/O, and other errors.
13. Multiple pending containers in one Pod perform one strict fresh lookup and
    protect the union of their allocations.
