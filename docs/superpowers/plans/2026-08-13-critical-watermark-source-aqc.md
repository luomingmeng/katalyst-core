...
func TestMemoryGuardConfigurationApplyConfiguration(t *testing.T) {
	...
	require.Equal(t, "low", conf.CriticalWatermarkSource)

	conf.ApplyConfiguration(newMemoryGuardDynamicConfig(&high))
	require.Equal(t, "high", conf.CriticalWatermarkSource)
...
Also assert `NewMemoryGuardOptions().CriticalWatermarkSource == "low"` and that
`AddFlags` registers `memory-advisor-critical-watermark-source`.
...
Add `CriticalWatermarkSource string` to dynamic `MemoryGuardOptions`, default it
to `low`, register the unchanged flag name, and validate in `ApplyTo`:
